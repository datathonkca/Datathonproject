from flask import Flask,request,render_template #request will help us to get data from the html input object
import numpy as np
app=Flask(__name__)
#App Routing helps us to map the URLs to a specific function that will handle the logic for that URL.
@app.route('/') #In our case here, the URL (“/”) is associated with the root URL.
...
def predict():
    ...
    string_prediction=" is likely to be discharged from the hospital on "+str(week_day_discharge) 
    #We add in that string the day, the month and the year for personalizing the output the the user    
    string_prediction= string_prediction + ", " +str(discharge_date_day) + " "+ str(discharge_month) + " "+ str(discharge_year)
